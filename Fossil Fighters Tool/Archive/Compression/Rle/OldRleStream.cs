...
public class OldRleStream : Stream
{
    ...
    public OldRleStream(Stream outputStream, RleStreamMode mode, int maxSizePerChunk, bool leaveOpen = false)
    {
    ...
    private void Decompress(ref SequenceReader<byte> reader)
    {
        if (!_hasHeader)
        {
            ...
            if ((rawData & 0x30) != 0x30) throw new InvalidDataException(string.Format(Localization.StreamIsNotCompressedBy, "RLE"));
