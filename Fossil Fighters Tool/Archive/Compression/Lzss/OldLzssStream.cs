...
public class OldLzssStream : Stream
{
    ...
    public OldLzssStream(Stream outputStream, LzssStreamMode mode, int maxSizePerChunk, bool leaveOpen = false)
    {
        ...
        _lookbackBuffer = ArrayPool<byte>.Shared.Rent(LookbackBufferSize);
        
        if (mode != LzssStreamMode.Compress) return;
    ...
    private void Decompress(ref SequenceReader<byte> reader)
    {
        if (!_hasHeader)
        {
            ...
            if (((rawData >> 4) & 0xF) != 0x1) throw new InvalidDataException(string.Format(Localization.StreamIsNotCompressedBy, "LZSS"));
